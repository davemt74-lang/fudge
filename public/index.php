    ...
    try {
        switch ($postAction) {
            ...
            case 'adjust_inventory':
                ...
                $ref=trim($_POST['catalog_ref']??'');$type=$_POST['item_type']??'';$id=(int)($_POST['item_id']??0);$qty=(float)($_POST['quantity_delta']??0);$reason=trim($_POST['reason']??'Manual Adjustment');
                if($ref!==''&&str_contains($ref,':')){[$type,$rawId]=explode(':',$ref,2);$id=(int)$rawId;}
                if (!$id || $qty==0 || !in_array($type,['ingredient','packaging'],true)) throw new RuntimeException('Select an item and enter a non-zero quantity.');
                $unit=$type==='ingredient'
                    ? (string)$db->scalar('SELECT inventory_unit FROM ingredients WHERE id=?',[$id])
                    : (string)$db->scalar('SELECT inventory_unit FROM packaging_items WHERE id=?',[$id]);
                if($unit==='') throw new RuntimeException('Inventory item not found.');
...
switch ($page) {
    ...
    case 'inventory':
        ...
        if(can('inventory.adjust')){$ings=$db->all('SELECT id,name,inventory_unit FROM ingredients WHERE is_active=1 ORDER BY name');$pkgs=$db->all('SELECT id,name,inventory_unit FROM packaging_items WHERE is_active=1 ORDER BY name');echo '<div class="card" style="margin-bottom:18px"><h2 class="section-title">Manual Adjustment / Initial Count</h2><form method="post" class="form-grid">'.Ui::csrf().'<input type="hidden" name="form_action" value="adjust_inventory"><label class="span-2">Inventory Item<select name="catalog_ref" required><option value="">Select item</option><optgroup label="Ingredients">';foreach($ings as $r)echo '<option value="ingredient:'.$r['id'].'">'.h($r['name']).' · '.h($r['inventory_unit']).'</option>';echo '</optgroup><optgroup label="Packaging">';foreach($pkgs as $r)echo '<option value="packaging:'.$r['id'].'">'.h($r['name']).' · '.h($r['inventory_unit']).'</option>';echo '</optgroup></select></label>';Ui::formField('quantity_delta','Quantity Change','','number',true,'step="0.001" placeholder="Use + for received/initial count, - for usage/waste"');Ui::formField('reason','Reason','Initial Count');echo '<div class="span-2"><button class="btn primary">Record Transaction</button></div></form></div>';}
