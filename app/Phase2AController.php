...
function handle_phase2a_page(string $page): never
{
    ...
    if ($page === 'purchasing') {
        ...
        if ($suggestions) {
            echo '<div class="card" style="margin-bottom:18px"><div class="table-head"><h2>Reorder Suggestions</h2><span class="badge warn">'.count($suggestions).' items below reorder point</span></div><table><thead><tr><th>Item</th><th>On Hand</th><th>Target</th><th>Suggested</th><th>Estimated Cost</th></tr></thead><tbody>';
            foreach($suggestions as $s) {
                echo '<tr><td><strong>'.h($s['name']).'</strong><div class="muted">'.h(ucfirst($s['item_type'])).'</div></td><td>'.number_format((float)$s['on_hand'],2).' '.h($s['unit']).'</td><td>'.number_format((float)$s['target_stock'],2).' '.h($s['unit']).'</td><td><strong>'.number_format((float)$s['suggested_quantity'],2).' '.h($s['unit']).'</strong></td><td>'.($s['estimated_cost']===null?'—':'$'.number_format((float)$s['estimated_cost'],2)).'</td></tr>';
            }
