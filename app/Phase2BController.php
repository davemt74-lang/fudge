        ...
        try {
            switch ($action) {
                ...
                case 'save_product_packaging':
                    ...
                    audit('product.packaging_saved','product',$productId,null,['packaging_item_id'=>$packagingId,'quantity'=>$qty,'unit'=>$unit]);
                ...
                case 'remove_product_packaging':
                    ...
                    audit('product.packaging_removed','product',(int)$component['product_id'],$component,null);
            ...
            if($recipe){
                ...
                if($versionId){
                    ...
                    if($version){
                        ...
                        $costByKey=[];if($preview)foreach($preview['details'] as $d)$costByKey[$d['component_type'].':'.$d['component_id']]=($costByKey[$d['component_type'].':'.$d['component_id']]??0)+$d['cost'];
                        foreach($items as $item){
                            $key=$item['component_type'].':'.$item['component_id'];
                            echo '<tr><td><strong>'.h($item['component_name']).'</strong></td><td>'.h(ucfirst($item['component_type'])).'</td><td>'.h($item['quantity'].' '.$item['unit']).'</td><td>$'.number_format((float)($costByKey[$key]??0),4).'</td><td>';
