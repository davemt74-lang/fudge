...
function handle_phase2b_page(string $page): never
{
    ...
    if ($page === 'costing') {
        ...
        foreach($impacts as $i){
            $dc=(float)$i['after_cogs']-(float)$i['before_cogs'];
            $dm=(float)$i['after_margin']-(float)$i['before_margin'];
            $complete=(int)$i['before_complete']===1&&(int)$i['after_complete']===1;
            echo '<tr><td>'.h($i['created_at']).'</td><td>'.h($i['product_name']).'</td><td>'.h(str_replace('_after','',$i['trigger_type'])).'<div class="muted">'.h($i['trigger_reference']).'</div></td><td>'.($dc>=0?'+':'').'USD '.number_format($dc,2).'</td><td>'.($dm>=0?'+':'').number_format($dm,1).' pp</td><td>'.($complete?'<span class="badge good">Complete</span>':'<span class="badge warn">Needs Pricing</span>').'</td></tr>';
        }
        if(!$impacts) echo '<tr><td colspan="6" class="empty">Cost changes appear here after supplier prices, recipes, or packaging BOMs change.</td></tr>';
        echo '</tbody></table></div>';
        ...
        foreach($priceChanges as $h){
            $old=(float)($h['old_unit_cost']??0);
            $new=(float)$h['unit_cost'];
            $delta=$new-$old;
            echo '<tr><td>'.h($h['effective_at']).'</td><td>'.h($h['item_name']).'</td><td>'.h($h['supplier']).'</td><td>USD '.number_format($old,4).'</td><td>USD '.number_format($new,4).'</td><td>'.($delta>=0?'+':'').'USD '.number_format($delta,4).'</td></tr>';
        }
