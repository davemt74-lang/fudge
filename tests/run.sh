...
echo "[2/6] Encryption round trip"
...
echo "[3/6] Schema contracts"
for table in users roles permissions ingredients packaging_items suppliers supplier_items supplier_price_history inventory_transactions recipes recipe_versions recipe_items flavors products customers orders production_batches llm_providers llm_credentials audit_log schema_migrations platform_meta; do
  grep -q "CREATE TABLE IF NOT EXISTS $table" "$ROOT/database/schema.sql" || { echo "Missing table: $table"; exit 1; }
done
echo "  PASS"

echo "[4/6] Permission contracts"
...
echo "[5/6] Migration manager contracts"
